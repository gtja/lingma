  ...
  <div>
    <div class="card">
      ...
      <table class="table" v-if="groupedItems.length">
        ...
        <tbody>
          <template v-for="group in groupedItems" :key="group.requirement">
            <tr class="group-row">
              <td>
                <input
                  type="checkbox"
              ...
              <td colspan="4">
                ...
                <div class="group-title">{{ group.requirement }}</div>
              </td>
            </tr>
            ...
            <tr v-for="item in group.children" :key="item.id" v-show="isGroupExpanded(group)">
              ...
              <td>
                <div><strong>#{{ item.id }}</strong> {{ item.title || '未命名用例' }}</div>
...
const groupedItems = computed(() => {
  ...
  for (const item of filteredItems.value) {
    const key = (item.requirements || '').trim() || '未命名需求';
    if (!groups.has(key)) {
      groups.set(key, {
        requirement: key,
        children: []
...
function isGroupExpanded(group) {
  return groupExpanded.value[group.requirement] !== false;
}

function toggleGroup(group) {
  const key = group.requirement;
...
watch(groupedItems, (groups) => {
  ...
  for (const g of groups) {
    next[g.requirement] = groupExpanded.value[g.requirement] !== false;
  }
  groupExpanded.value = next;
}, { immediate: true });
